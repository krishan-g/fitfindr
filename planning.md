...
I plan to use Claude, giving it the Tools and Error Handling sections. I expect it to produce working tools with proper error handling. To verify that the tools match the spec, I will test each tool against three cases.

**Milestone 4 — Planning loop and state management:**
I plan to use Claude, giving it the Planning Loop, State Management, Architecture, and A Complete Interaction (Step by Step) sections. I expect it to produce an agent that can successfully pass state between tool uses and make decisions according to the planning loop. To verify that this matches the spec, I will check that the state is correctly updated, and that the agent calls the specific tools when appropriate (it shouldn't unconditionally call tools).
